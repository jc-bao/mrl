...
class DDPGPolicy(BasePolicy):
    def __init__(self, config, actor, actor_opt, critic, critic_opt, replay_buffer) -> None:
        super().__init__()
        # parameters
        self.config = config
    ...
    def forward(self, state, training=True, random_explore=False, greedy=False):
        if training:
            if self.config.get('initial_explore') and len(self.replay_buffer) < self.config.initial_explore:
                res = np.array([self.config.action_space.sample()
                                for _ in range(self.config.num_envs)])
                return res  # TODO 3 remove buffer
            elif hasattr(self, 'ag_curiosity'):  #  1 curiosity
