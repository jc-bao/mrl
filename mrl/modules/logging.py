...
class Logger(mrl.Module):
    ...
    def __init__(self, average_every=10, wandb_config={}):
    ...
    def add_histogram(self, tag, values, log_every=1000, step=None, **kwargs):
        ...
        if step - self.last_log_step[tag] >= log_every:
            ...
            # if hasattr(self, 'wandb_run') > 0:
            #     data = [[s] for s in self.numpy(values)]
            #     table = wandb.Table(data=data, columns=[tag])
            #     gram = wandb.plot.histogram(table, tag)
                # self.wandb_run.log({tag: gram}, step=step)
    ...
    def add_embedding(self, tag, values, log_every=1000, step=None, **kwargs):
        ...
        if step - self.last_log_step[tag] >= log_every:
            ...
            #     table = wandb.Table(
            #         columns=np.arange(values.shape[0]),
            #         data=values
            #     )
            #     self.wandb_run.log({tag: table}, step=step)
